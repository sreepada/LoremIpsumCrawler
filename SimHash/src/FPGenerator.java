...
public final class FPGenerator {
   ...
   * will be returned. Requires that <code>polynomial</code> is an irreducible
   * polynomial of degree <code>degree</code> (the array
  ...
  /**
   * Return a value equal (mod <code>polynomial</code>) to <code>fp</code> and
   * of degree less than <code>degree</code>.
   */
  ...
  /**
   * Extends <code>f</code> with lower eight bits of <code>v</code> with
   * <em>out</em> full reduction. In other words, returns a polynomial that is
   * equal (mod <code>polynomial</code>) to the desired fingerprint but may be
   * of higher degree than the desired fingerprint.
   */
  ...
  /**
   * Extends <code>f</code> with lower sixteen bits of <code>v</code>. Does not
   * reduce.
   */
  ...
  /**
   * Extends <code>f</code> with (all bits of) <code>v</code>. Does not reduce.
   */
  ...
  /**
   * Compute fingerprint of (all bits of) "n" characters of "buf" starting from
   * "buf[i]". Requires "[i, i+n)" is in bounds.
   */
  ...
  /**
   * Extends fingerprint <code>f</code> by adding "n" bytes of "buf" starting
   * from "buf[start]". Result is reduced. Requires "[i,&nbsp;i+n)" is in
   * bounds.
   */
  ...
  /**
   * Extends fingerprint <code>f</code> by adding (all bits of) "n" characters
   * of "buf" starting from "buf[i]". Result is reduced. Requires
   * "[i,&nbsp;i+n)" is in bounds.
   */
  ...
  /**
   * Extends fingerprint <code>f</code> by adding (all bits of) the characters
   * of "s". Result is reduced.
   */
  ...
  /**
   * Extends fingerprint <code>f</code> by adding the lower eight bits of the
  ...
  /**
   * Extends fingerprint <code>f</code> by adding the lower eight bits of "n"
  ...
  /**
   * Result of reducing certain polynomials. Specifically, if <code>f(S)</code>
   * is bit string <code>S</code> interpreted as a polynomial,
   * <code>f(ByteModTable[i][j])</code> equals
   * <code>mod(x^(127&nbsp;-&nbsp;8*i)&nbsp;*&nbsp;f(j),&nbsp;P)</code>.
   */
  private long[][]  ByteModTable;

  /**
   * Create a fingerprint generator. The fingerprints generated will have degree
   * <code>degree</code> and will be generated by <code>polynomial</code>.
   * Requires that <code>polynomial</code> is an irreducible polynomial of
   * degree <code>degree</code> (the array <code>polynomials</code> contains
  ...
  /**
   * Array of irreducible polynomials. For each degree <code>d</code> between 1
   * and 64 (inclusive), <code>polynomials[d][i]</code> is an irreducible
   * polynomial of degree <code>d</code>. There are at least two irreducible
